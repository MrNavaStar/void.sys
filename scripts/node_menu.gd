...
var last_event_pos2D: Vector2 = Vector2.INF
# The time of the last event in seconds since engine start.
var last_event_time: float = -1.0

@onready var node_viewport: SubViewport = $SubViewport
@onready var node_quad: MeshInstance3D = $Menu
@onready var node_area: Area3D = $Menu/Area3D
...
func _mouse_input_event(
	...
	var quad_mesh_size: Vector2 = (node_quad.mesh as PlaneMesh).size
	...
	if event is InputEventMouseMotion:
		...
		# the previous position from the new position. This will give us the distance the event traveled from prev_pos.
			...
			(event as InputEventMouseMotion).velocity = (
				(event as InputEventMouseMotion).relative / (now - last_event_time)
			)
